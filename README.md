# Trabajo Final Integrador de Programación III – Grupo D

Proyecto desarrollado bajo la guía de los profesores **Cristian Faure** y **Nacho Novello**.

## ¡Bienvenidos!

![gif](https://media3.giphy.com/media/v1.Y2lkPTc5MGI3NjExY29veXk3aTFvNHQ2bGQ3YXUzYmhwYmttdXh2cXI0bHI4eWZsMHdqNiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/xT9IgG50Fb7Mi0prBC/giphy.gif)

Estamos comenzando la construcción de nuestra **API REST** para la materia **Programación 3**.

### Objetivo del Proyecto en desarrollo

Sistema de gestión de turnos médicos que permite administrar especialidades, médicos, pacientes y turnos.

### Funcionalidades

- BREAD completo de Especialidades
- Versionado de API (`/api/v1`)
- Soft delete

### Equipo

- Vanesa Micaela Molina
- Enzo Sebastian Guido
- Alethia Azul Gerez Medina
- Carlos Gabrovich
- Paola Peñalva Rebaquela
- Camila Battagliotti
